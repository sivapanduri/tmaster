...
recordRoutes.route("/record/:id").get(function (req, res) {
  ...
  db_connect.collection("countries").findOne(myquery, function (err, result) {
    if (err) throw err;
    res.json(result);
  });
});
...
recordRoutes.route("/update/:id").post(async (req, response) => {
  ...
  db_connect.collection("countries").updateOne(myquery,newvalues,function(err,res){
    if(err) throw err;
